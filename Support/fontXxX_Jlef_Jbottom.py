...
def generate_font_array(font_path, size, output_filename, font_name_override=None):
    ...
    All characters are strictly left-justified and bottom-justified.
    """
    if size % 8 != 0:
    ...
    def render_char(c):
        ...
        # Calculate bounding box for absolute alignment
        bbox = font.getbbox(c)
        if bbox:
            char_h = bbox[3] - bbox[1]
            
            # FIX 1: Strict left-justification (X = 0) by counter-shifting the font's internal offset
            x_shift = -bbox[0]
            
            # FIX 2: Strict bottom-justification (Y = bottom edge)
            # Shifts the text so the lowest pixel hits the absolute bottom row of the square canvas
            y_shift = (glyph_h - char_h) - bbox[1]
        else:
            x_shift, y_shift = 0, 0
            
        # Render the character left and bottom justified
    ...
    with open(output_filename, "w") as f:
        f.write(f"// Generated {size}x{size} font grid ({bytes_per_row} bytes per row) - Left & Bottom Justified\n")
...
generate_font_array(FONT_FILE, size=48, output_filename="font48x48.txt")
generate_font_array(FONT_FILE, size=64, output_filename="font64x64.txt")
