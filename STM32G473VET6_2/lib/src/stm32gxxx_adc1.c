...
static inline void _adc1_init(void) {
	...
	exe()->clear_reg(&dev()->analog->adc1->CR, ADC_CR_DEEPPWD);
	/* Ativar o regulador de tensão interno do ADC1 */
	exe()->set_reg(&dev()->analog->adc1->CR, ADC_CR_ADVREGEN);
...
static inline void _adc1_enable(void) {
	...
	exe()->set_reg(&dev()->analog->adc1->ISR, ADC_ISR_ADRDY);
	/* Ligar o ADC e aguardar o flag de pronto */
	exe()->set_reg(&dev()->analog->adc1->CR, ADC_CR_ADEN);
...
static inline int _adc1_disable(void)
{
    ...
    {
        exe()->set_reg(&adc->CR, ADC_CR_ADSTP);
    ...
    {
        exe()->set_reg(&adc->CR, ADC_CR_JADSTP);
    ...
    // Request disable
    exe()->set_reg(&adc->CR, ADC_CR_ADDIS);
...
static inline void _adc1_cal_single(void) {
	...
	exe()->set_reg(&dev()->analog->adc1->CR, ADC_CR_ADCAL);
...
static void adc1_temp_init(void) {
	...
	exe()->set_reg(&dev()->sys->rcc->AHB2ENR, RCC_AHB2ENR_ADC12EN);
    ...
    exe()->set_reg( &dev()->analog->adc12_common->CCR, ADC_CCR_VSENSESEL );
...
{
    exe()->set_reg(&adc->CR, ADC_CR_ADCAL);
...
{
    exe()->set_reg(&adc->CR, ADC_CR_JADSTART);
}
...
{
    exe()->set_reg(&adc->CR, ADC_CR_ADSTART);
}
