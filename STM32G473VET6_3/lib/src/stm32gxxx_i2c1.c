...
static inline void _i2c1_modify_cr1_protected(void (*modify_func)(void)) {
    uint32_t is_enabled = exe()->get_field_value(dev()->comm->i2c1->CR1.reg, I2C_CR1_PE, I2C_CR1_PE_Pos);
    if (is_enabled) {
        exe()->clear_reg(&dev()->comm->i2c1->CR1.reg, I2C_CR1_PE);
        while(exe()->get_field_value(dev()->comm->i2c1->CR1.reg, I2C_CR1_PE, I2C_CR1_PE_Pos));
    }
    modify_func();
    if (is_enabled) {
        exe()->set_reg(&dev()->comm->i2c1->CR1.reg, I2C_CR1_PE);
    }
}
...
static uint8_t _i2c1_get_flag(uint32_t flag_mask) {
    ...
    return (dev()->comm->i2c1->ISR.reg & flag_mask) ? 1U : 0U;
}
...
static uint8_t _i2c1_get_status_flag(uint32_t msk) {
	...
	return exe()->get_field_value(dev()->comm->i2c1->ISR.reg, msk, 0) ? 1U : 0U;
}
...
static uint8_t _i2c1_wait_status_flag(uint32_t msk, uint8_t expected_state, uint32_t timeout_loops) {
	...
	while (_i2c1_get_status_flag(msk) != expected_state) {
		...
		if (msk != I2C_ISR_NACKF && _i2c1_get_status_flag(I2C_ISR_NACKF)) {
			exe()->set_reg(&dev()->comm->i2c1->ICR.reg, I2C_ICR_NACKCF); // Clear NACK flag
			return 0;
		}
	}
	return 1; // Success
...
uint8_t _i2c1_is_enabled(void) {
	return exe()->get_field_value(dev()->comm->i2c1->CR1.reg,I2C_CR1_PE,I2C_CR1_PE_Pos);
}
...
static void i2c1_digital_filter(uint8_t filter) {
	...
	exe()->write_field_value(&dev()->comm->i2c1->CR1.reg,I2C_CR1_DNF,I2C_CR1_DNF_Pos,filter);
...
static void i2c1_analog_filter_enable(void) {
	...
	exe()->clear_reg(&dev()->comm->i2c1->CR1.reg, I2C_CR1_ANFOFF);
...
static void i2c1_analog_filter_disable(void) {
	...
	exe()->set_reg(&dev()->comm->i2c1->CR1.reg, I2C_CR1_ANFOFF);
...
static void i2c1_software_reset(void) {
	exe()->set_reg(&dev()->comm->i2c1->CR1.reg,I2C_CR1_SWRST);
}
static void i2c1_clock_stretch_enable(void) {
	exe()->clear_reg(&dev()->comm->i2c1->CR1.reg,I2C_CR1_NOSTRETCH);
}
static void i2c1_clock_stretch_disable(void) {
	exe()->set_reg(&dev()->comm->i2c1->CR1.reg,I2C_CR1_NOSTRETCH);
}
static void i2c1_general_call_enable(void) {
	exe()->set_reg(&dev()->comm->i2c1->CR1.reg,I2C_CR1_GCEN);
}
static void i2c1_general_call_disable(void) {
	exe()->clear_reg(&dev()->comm->i2c1->CR1.reg,I2C_CR1_GCEN);
}
static void i2c1_pec_enable(void) {
	exe()->set_reg(&dev()->comm->i2c1->CR1.reg,I2C_CR1_PECEN);
}
static void i2c1_pec_disable(void) {
	exe()->clear_reg(&dev()->comm->i2c1->CR1.reg,I2C_CR1_PECEN);
}
...
static void i2c1_start(void) {
	...
	exe()->write_field_encoded(&dev()->comm->i2c1->CR2.reg, 0xFFFFFFFFU, final_hardware_cmd);
...
static uint8_t i2c1_get_start(void) {
	return exe()->get_field_value(dev()->comm->i2c1->CR2.reg, I2C_CR2_START, I2C_CR2_START_Pos);
}
static void i2c1_stop(void) {
	// STOP can be asserted directly to hardware safely to terminate/abort sequences
	exe()->set_reg(&dev()->comm->i2c1->CR2.reg, I2C_CR2_STOP);
}

/*******************  Bit definition for I2C_OAR1 register  ******************/
static void i2c1_own_address(uint8_t address) {
	exe()->write_field_value(&dev()->comm->i2c1->OAR1.reg,I2C_OAR1_OA1,I2C_OAR1_OA1_Pos,address);
}
static void i2c1_own_address_10bit_mode_enable(void) {
	exe()->set_reg(&dev()->comm->i2c1->OAR1.reg,I2C_OAR1_OA1MODE);
}
static void i2c1_own_address_10bit_mode_disable(void) {
	exe()->clear_reg(&dev()->comm->i2c1->OAR1.reg,I2C_OAR1_OA1MODE);
}
static void i2c1_own_address_enable(void) {
	exe()->set_reg(&dev()->comm->i2c1->OAR1.reg,I2C_OAR1_OA1EN);
}
static void i2c1_own_address_disable(void) {
	exe()->clear_reg(&dev()->comm->i2c1->OAR1.reg,I2C_OAR1_OA1EN);
}
/*******************  Bit definition for I2C_TIMINGR register *******************/
static void i2c1_low_period(uint8_t ll) {
	exe()->write_field_value(&dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SCLL,I2C_TIMINGR_SCLL_Pos,ll);
}
static void i2c1_high_period(uint8_t hh) {
	exe()->write_field_value(&dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SCLH,I2C_TIMINGR_SCLH_Pos,hh);
}
static void i2c1_hold_timing(uint8_t hold) {
	exe()->write_field_value(&dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SDADEL,I2C_TIMINGR_SDADEL_Pos,hold & 0xF);
}
static void i2c1_setup_timing(uint8_t setup) {
	exe()->write_field_value(&dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SCLDEL,I2C_TIMINGR_SCLDEL_Pos,setup & 0xF);
}
static void i2c1_timing_prescaler(uint8_t prescaler) {
	exe()->write_field_value(&dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_PRESC,I2C_TIMINGR_PRESC_Pos,prescaler & 0xF);
}
static uint8_t i2c1_get_low_period(void) {
	return exe()->get_field_value(dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SCLL,I2C_TIMINGR_SCLL_Pos);
}
static uint8_t i2c1_get_high_period(void) {
	return exe()->get_field_value(dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SCLH,I2C_TIMINGR_SCLH_Pos);
}
static uint8_t i2c1_get_hold_timing(void) {
	return exe()->get_field_value(dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SDADEL,I2C_TIMINGR_SDADEL_Pos);
}
static uint8_t i2c1_get_setup_timing(void) {
	return exe()->get_field_value(dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_SCLDEL,I2C_TIMINGR_SCLDEL_Pos);
}
static uint8_t i2c1_get_timing_prescaler(void) {
	return exe()->get_field_value(dev()->comm->i2c1->TIMINGR.reg,I2C_TIMINGR_PRESC,I2C_TIMINGR_PRESC_Pos);
}

/******************* Bit definition for I2C_TIMEOUTR register *******************/
static void i2c1_bus_timeout(uint16_t timeout) {
	exe()->write_field_value(&dev()->comm->i2c1->TIMEOUTR.reg,I2C_TIMEOUTR_TIMEOUTA,I2C_TIMEOUTR_TIMEOUTA_Pos,timeout & 0x0FFF);
}
static void i2c1_idle_timeout_detect_enable(void) {
	exe()->set_reg(&dev()->comm->i2c1->TIMEOUTR.reg,I2C_TIMEOUTR_TIDLE);
}
static void i2c1_idle_timeout_detect_disable(void) {
	exe()->clear_reg(&dev()->comm->i2c1->TIMEOUTR.reg,I2C_TIMEOUTR_TIDLE);
}
static void i2c1_timeout_enable(void) {
	exe()->set_reg(&dev()->comm->i2c1->TIMEOUTR.reg,I2C_TIMEOUTR_TIMOUTEN);
}
static void i2c1_timeout_disable(void) {
	exe()->clear_reg(&dev()->comm->i2c1->TIMEOUTR.reg,I2C_TIMEOUTR_TIMOUTEN);
}

/******************  Bit definition for I2C_ISR register  *********************/
static uint32_t i2c1_status(void) { // ACK NACK & ERROR FLAGS
	return dev()->comm->i2c1->ISR.reg;
}

/******************  Bit definition for I2C_PECR register  *********************/
static uint8_t i2c1_get_pecr(void) {
	return exe()->get_field_value(dev()->comm->i2c1->PECR.reg,I2C_PECR_PEC,I2C_PECR_PEC_Pos);
}

/******************  Bit definition for I2C_RXDR register  *********************/
static uint8_t i2c1_get_rxdata(void) {
	return exe()->get_field_value(dev()->comm->i2c1->RXDR.reg,I2C_RXDR_RXDATA,I2C_RXDR_RXDATA_Pos);
}

/******************  Bit definition for I2C_TXDR register  *********************/
static void i2c1_set_txdata(uint8_t data) {
	exe()->write_field_value(&dev()->comm->i2c1->TXDR.reg,I2C_TXDR_TXDATA,I2C_TXDR_TXDATA_Pos, data);
}
...
static uint8_t i2c1_is_idle(void) {
	uint32_t isr_value = dev()->comm->i2c1->ISR.reg;
...
static void i2c1_calculate_and_apply_timing(i2c_bus_speed_t target_bus_speed_hz) {
	...
	exe()->write_field_encoded(&dev()->comm->i2c1->TIMINGR.reg, 0xFFFFFFFFU, timingr_value);
...
static uint8_t i2c1_write_buffer(uint16_t device_id, uint8_t* p_data, uint8_t length) {
	...
	exe()->set_reg(&dev()->comm->i2c1->ICR.reg, 0x3F38U);
