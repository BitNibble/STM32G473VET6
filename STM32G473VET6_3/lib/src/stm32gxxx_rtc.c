...
static uint8_t RTC_get_year(void) {
    RTC_Wait_sync();
    uint32_t dr = RTC->DR;
    uint8_t t = (uint8_t)exe()->get_field(dr, RTC_DR_YT_Msk, RTC_DR_YT_Pos);
    uint8_t u = (uint8_t)exe()->get_field(dr, RTC_DR_YU_Msk, RTC_DR_YU_Pos);
...
static uint8_t RTC_get_month(void) {
    RTC_Wait_sync();
    uint32_t dr = RTC->DR;
    uint8_t t = (uint8_t)exe()->get_field(dr, RTC_DR_MT_Msk, RTC_DR_MT_Pos);
    uint8_t u = (uint8_t)exe()->get_field(dr, RTC_DR_MU_Msk, RTC_DR_MU_Pos);
    return (_rtc_bcd2dec(t) * 10) + _rtc_bcd2dec(u);
}

static uint8_t RTC_get_weekday(void) {
    RTC_Wait_sync();
    return (uint8_t)exe()->get_field(RTC->DR, RTC_DR_WDU_Msk, RTC_DR_WDU_Pos);
}

static uint8_t RTC_get_day(void) {
    RTC_Wait_sync();
    uint32_t dr = RTC->DR;
    uint8_t t = (uint8_t)exe()->get_field(dr, RTC_DR_DT_Msk, RTC_DR_DT_Pos);
    uint8_t u = (uint8_t)exe()->get_field(dr, RTC_DR_DU_Msk, RTC_DR_DU_Pos);
    return (_rtc_bcd2dec(t) * 10) + _rtc_bcd2dec(u);
}
...
static uint8_t RTC_get_hour(void) {
    RTC_Wait_sync();
    uint32_t tr = RTC->TR;
    uint8_t t = (uint8_t)exe()->get_field(tr, RTC_TR_HT_Msk, RTC_TR_HT_Pos);
    uint8_t u = (uint8_t)exe()->get_field(tr, RTC_TR_HU_Msk, RTC_TR_HU_Pos);
    return (_rtc_bcd2dec(t) * 10) + _rtc_bcd2dec(u);
}
...
static uint8_t RTC_get_minute(void) {
    RTC_Wait_sync();
    uint32_t tr = RTC->TR;
    uint8_t t = (uint8_t)exe()->get_field(tr, RTC_TR_MNT_Msk, RTC_TR_MNT_Pos);
    uint8_t u = (uint8_t)exe()->get_field(tr, RTC_TR_MNU_Msk, RTC_TR_MNU_Pos);
    return (_rtc_bcd2dec(t) * 10) + _rtc_bcd2dec(u);
}
...
static uint8_t RTC_get_second(void) {
    RTC_Wait_sync();
    uint32_t tr = RTC->TR;
    uint8_t t = (uint8_t)exe()->get_field(tr, RTC_TR_ST_Msk, RTC_TR_ST_Pos);
    uint8_t u = (uint8_t)exe()->get_field(tr, RTC_TR_SU_Msk, RTC_TR_SU_Pos);
    return (_rtc_bcd2dec(t) * 10) + _rtc_bcd2dec(u);
}
...
static void RTC_set_weekday(uint8_t weekday) {
    ...
    uint32_t dr = RTC->DR;
    exe()->write_field(&dr, RTC_DR_WDU_Msk, RTC_DR_WDU_Pos, weekday);
    RTC_Set_dr(dr);
}
...
static void RTC_Wait_sync(void) {
    ...
    while(!exe()->get_field(RTC->ICSR, RTC_ICSR_RSF_Msk, RTC_ICSR_RSF_Pos) && --timeout);
}
...
static void RTC_Set_tr(uint32_t value) {
    ...
    uint32_t timeout = RTC_INIT_TIMEOUT;
    while(!exe()->get_field(RTC->ICSR, RTC_ICSR_INITF_Msk, RTC_ICSR_INITF_Pos) && --timeout);
